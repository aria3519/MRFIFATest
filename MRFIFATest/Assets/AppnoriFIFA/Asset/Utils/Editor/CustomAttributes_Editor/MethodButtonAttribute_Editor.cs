...
namespace Jisu.Utils
{
    ...
    public class MethodButtonAttributeDrawer : PropertyDrawer
    {
        ...
        {
            Color eBtnColor = Atr.ButtonColor;

            Color textColor = Atr.TextColor;
            Color btnColor = eBtnColor;
            ...
            btnWidthRatio = Mathf.Clamp(btnWidthRatio, 0f, 1f); // 비율 : 0 ~ 1 사이로 제한
            ...
            var btnRect = new Rect(btnX, currentY, btnWidth, BtnHeight);
            ...
            if (method == null)
            {
                var errRect = new Rect(position.x, currentY, position.width, BtnHeight);
                EditorGUI.HelpBox(errRect, $"Error : Not Existed Method Name \"{methodName}\"", MessageType.Error);
            }
            else
            {
                // 배경 색상 지정
                if (!eBtnColor.Equals(Color.black))
                {
                    // 버튼 색상은 내부적으로 1/3 정도로 적용되는듯
                    // 그래서 3배를 해줌
                    var newBtnColor = new Color(btnColor.r * 3f, btnColor.g * 3f, btnColor.b * 3f, 1.0f);
