...
namespace FStudio.MatchEngine
{
    ...
    public class TransFormManager : SceneObjectSingleton<TransFormManager>
    {
        ...
        private void ChangeMapSize()
        {
            ...
            if (Time.timeScale != 0)
            {
                if(_ballShadow.activeSelf == false)
                    _ballShadow.SetActive(true);
                return;
            }
            if(_ballShadow.activeSelf == true)
                    _ballShadow.SetActive(false);
            ...
            else if (ControllerManager._ButtonSt.GripR == true
               )
            {
                //&& checkTarget == false
...
*/
        ...
        public void TransFormBallPos(Vector3 pos)
        {
            ...
            tempPos.y =_transMap.transform.position.y+0.001f;
        ...
        public void OnOffButton(bool check)
        {
            if (check)
            {
