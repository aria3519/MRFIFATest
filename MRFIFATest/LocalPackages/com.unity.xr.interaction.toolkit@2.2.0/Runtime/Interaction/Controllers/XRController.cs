...
namespace UnityEngine.XR.Interaction.Toolkit
{
    ...
    public class XRController : XRBaseController
    {
        ...
        public static Vector3 posVec = Vector3.zero;
        public static Quaternion rotQ = Quaternion.identity;

        public static float value_haptic = 1f;
        ...
        protected override void UpdateTrackingInput(XRControllerState controllerState)
        {
...
#endif
            {
                if (inputDevice.TryGetFeatureValue(CommonUsages.trackingState, out var trackingState))
                {
                    ...
                    {
                        //controllerState.position = devicePosition + transform.right * posVec.x + transform.up * posVec.y + transform.forward * posVec.z;
                        controllerState.position = devicePosition + transform.up * posVec.y + transform.forward * posVec.z;                    }
                    ...
                    {
                        controllerState.rotation = deviceRotation * rotQ;
                    }
                }
            }
        }
        ...
        {
#if PICO_PLATFORM
            Unity.XR.PXR.PXR_Input.Controller controller = (m_ControllerNode == XRNode.RightHand ? Unity.XR.PXR.PXR_Input.Controller.RightController : Unity.XR.PXR.PXR_Input.Controller.LeftController);
            Unity.XR.PXR.PXR_Input.SetControllerVibration(amplitude * value_haptic, (int)(duration * 1000), controller);
            return true;
#else
            if (inputDevice.TryGetHapticCapabilities(out var capabilities) &&
                capabilities.supportsImpulse)
            {
                return inputDevice.SendHapticImpulse(0u, amplitude * value_haptic, duration);
            }
            return false;
#endif
        }
...
#if PICO_PLATFORM
            Unity.XR.PXR.PXR_Input.Controller controller = (m_ControllerNode == XRNode.RightHand ? Unity.XR.PXR.PXR_Input.Controller.RightController : Unity.XR.PXR.PXR_Input.Controller.LeftController);
            Unity.XR.PXR.PXR_Input.SetControllerVibration(0f, 0, controller);
#else
            if (inputDevice.TryGetHapticCapabilities(out var capabilities) &&
                capabilities.supportsImpulse)
            {
                inputDevice.SendHapticImpulse(0u, 0f, 0f);
            }
#endif
        }
    }
